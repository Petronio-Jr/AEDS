...
void leitura(mtz v, int maxlinhas, int maxcolunas, int maxcinza, ifstream& arq) { //função pra ler os dados do arquivo numa matriz
    ...
    for (int i = 0; i < maxlinhas; i++) { //começa na primeira linha

        int* inicio = (int*)v + maxcolunas * i; //primeiro elemento 
        int* fim = (int*)v + maxcolunas * i + maxcolunas; //ultimo elemento

        while (inicio < fim) {

            arq >> *inicio; //atribuição da matriz com os valores do aquivo
            inicio++; //avança a coluna
        }
    }
    arq.close();
} 
...
int main() {
    ...
    if (opimg == 1 || opimg == 2) {
        if (!arquivo.is_open()) {
            cout << "Algo deu errado na abertura do arquivo!\n"; //checando se o arquivo foi aberto
            return 1;
        }
    }
    ...
    if (tipo != "P2") { //verificando se o arquivo tem o formato correto
            cout << "O arquivo não é compatível";
    }
